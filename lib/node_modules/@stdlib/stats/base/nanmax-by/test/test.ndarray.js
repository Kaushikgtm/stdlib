...
tape( 'the function has an arity of 6', function test( t ) {
	t.strictEqual( nanmaxBy.length, 6, 'has expected arity' );
	t.end();
});
...
tape( 'the function supports a `stride` parameter (accessors)', function test( t ) {
	...
	N = floor( x.length / 2 );
	v = nanmaxBy( N, toAccessorArray( x ), 2, 0, accessor );
...
tape( 'the function supports a negative `stride` parameter (accessors)', function test( t ) {
	...
	N = floor( x.length / 2 );
	v = nanmaxBy( N, toAccessorArray( x ), -2, 8, accessor );
...
tape( 'the function supports providing a callback execution context', function test( t ) {
	...
	nanmaxBy( x.length, x, 1, 0, accessor, ctx );
